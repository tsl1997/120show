...
(function () {
  ...
  function nap() {
    const s = LJ.Sys.state;
    LJ.Engine.advance(1, {});
    s.player.energy = LJ.Engine.clamp(s.player.energy + 15, 0, 100);
    LJ.Engine.log('你靠着墙眯了一小时，缓过劲来。');
    LJ.Engine.toast('☕ 小憩1小时，体力+15');
    LJ.Sys.save(); LJ.UI.renderAll();
  }

  // 等待跳过时间（长途按天自动吃喝）
  function waitHours(hours) {
    const s = LJ.Sys.state;
    let left = hours;
    while (left > 0) {
      const step = Math.min(left, 24);
      LJ.Engine.advance(step, {});
      if (step >= 24) autoNourish(true);
      left -= step;
    }
    LJ.Engine.log(`你等待了${hours}小时，时间悄然流逝。`);
    LJ.Engine.toast(`⏳ 过了${hours}小时`);
    LJ.Sys.save(); LJ.UI.renderAll();
  }
  ...
  function autoNourish(autoBuy) {
    ...
    const findFood = () => {
      let slot = s.home[s.world].find((x) => LJ.Items[x.id] && LJ.Items[x.id].consume && (LJ.Items[x.id].consume.hunger || 0) >= 15);
      if (slot) return slot;
      return s.bag.find((x) => LJ.Items[x.id] && LJ.Items[x.id].consume && (LJ.Items[x.id].consume.hunger || 0) >= 15);
    };
    const findDrink = () => {
      let slot = s.home[s.world].find((x) => LJ.Items[x.id] && LJ.Items[x.id].consume && (LJ.Items[x.id].consume.thirst || 0) >= 15);
      if (slot) return slot;
      return s.bag.find((x) => LJ.Items[x.id] && LJ.Items[x.id].consume && (LJ.Items[x.id].consume.thirst || 0) >= 15);
    };
    if (s.player.hunger < 55) {
      ...
      if (slot) {
        ...
        removeOne(s.home[s.world].includes(slot) ? s.home[s.world] : s.bag, slot.id);
  ...
  function renderHomeEating(content) {
    const s = LJ.Sys.state;
    content.appendChild(el(`<div class="panel-note">家里储存的食物与水。可以花小钱烧一壶白开水。</div>`));
    const freeWater = el(`<button class="primary">♨️ 烧一壶白开水（15分钟 · 口渴+30）</button>`);
    ...
    const eatables = s.home[s.world].filter((x) => LJ.Items[x.id] && LJ.Items[x.id].consume);
    if (!eatables.length) {
      content.appendChild(el(`<div class="empty">家里没有可吃喝的东西。去饭店或商店补充吧。</div>`));
      return;
    }
  ...
  const OLD_MEALS = [
    ...
    { id: 'o_pork', name: '红烧肉套餐', cost: 1.5, hunger: 65, thirst: 6, spirit: 12, time: 0.6, desc: '国营饭店的招牌硬菜。' },
  ...
  function renderDining(content) {
    ...
    const grid = el(`<div class="cards">${meals.map((m) => {
      ...
      return `<div class="card"><h3>${m.name}</h3><p>${m.desc}<br>${m.cost}元 · ${Math.round(m.time * 60)}分钟</p><div class="trade-ops"><button data-meal="${m.id}" ${afford ? '' : 'disabled'}>点餐</button></div></div>`;
    }).join('')}</div>`);
  ...
  LJ.Survival = { rest, nap, waitHours, stroll, useItem, autoNourish, renderHomeEating, renderDining };
})();
