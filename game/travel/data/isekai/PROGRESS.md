...
> 流程：每作品 = spots分册 + stories分册(人物+事件) + gifts分册 + 组装器(isekai_xxx.js) + index.html挂载 + check_data校验。
> 分册window数组命名：SP/ST/GF 加作品前缀。系列收藏按题材归入 series_isekai_b/c/d.js。
...
- ✅ slamdunk 灌篮高手：97条（sl_01~03）
- ✅ sailormoon 美少女战士：84条（sm_01~03）
- ✅ db 龙珠（新建）：93条（db_01~03）+系列「湘北先发五虎」「七颗龙珠」「内部水手战士」(series_isekai_b.js)
- ✅ onepiece 海贼王：607条（op_01~op_10）+系列「草帽海贼团伙伴」
- ✅ naruto 火影忍者：229条（na_01~na_09）+系列「历代火影」
- ✅ conan 柯南：177条（co_01~co_08）+系列「少年侦探团」「黑衣组织代号」
