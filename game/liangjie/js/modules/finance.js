/* 两界搬运工 · 金融系统
 * 银行存取（每日计息）、多币种外汇（现代年度5万美元购汇额度；旧时外汇券）、A股模拟。
 */
(function () {
  'use strict';
  ...
  function tickInterest(world) {
    ...
    if (interest > 0.001) s.bank[world].CNY = Math.round((s.bank[world].CNY + interest) * 100) / 100;
  }
  ...
  function renderBank(content) {
    ...
    const box = el(`<div class="fx-box">
      <h3>存 / 取</h3>
      <div class="row">
        <input type="number" id="bankAmt" min="0" value="100">
        <button id="deposit">存入</button><button id="withdraw">取出</button>
      </div>
    </div>`);
  ...
  function renderModernFX(content) {
    const s = LJ.Sys.state;
    const quotaLeft = Math.max(0, 50000 - (s.money.fxQuota.USD || 0));
    ...
    const box = el(`<div class="fx-box">
      ...
      <div class="row">
        ...
        <input type="number" id="fxAmt" min="0" value="100">
        <button id="fxBuy">用人民币购买</button><button id="fxSell">卖出外币换人民币</button>
      </div>
    ...
    $('#fxBuy', box).addEventListener('click', () => {
      ...
      const usdEq = cny / 7.2;
      if ((s.money.fxQuota.USD || 0) + usdEq > 50000) { LJ.Engine.toast('超过年度购汇额度（5万美元）。'); return; }
      s.money.modern.CNY -= cny;
      s.money.modern[cur] = (s.money.modern[cur] || 0) + amt;
      s.money.fxQuota.USD = (s.money.fxQuota.USD || 0) + usdEq;
  ...
  function renderOldFX(content) {
    ...
    content.appendChild(el(`<div class="fx-box">
      ...
      <div class="row">
        <select id="fxOldCur"><option value="USD">美元（1.5元/美元）</option><option value="HKD">港币（0.2元/港币）</option><option value="JPY">日元（0.007元/日元）</option></select>
        <input type="number" id="fxOldAmt" min="0" value="1">
        <button id="fxOldToCny">外币→人民币</button>
      </div>
      <div class="row">用人民币买外汇券：<input type="number" id="fecAmt" min="0" value="10"><button id="fecBuy">买外汇券</button><button id="fecSell">外汇券→人民币</button>
        <span class="tiny">当前外汇券：${LJ.Engine.money(s.money.old.FEC)}</span></div>
    </div>`));
  ...
  function marketOpen() {
    ...
    return (hour >= 9.5 && hour <= 11.5) || (hour >= 13 && hour <= 15);
  }

  function renderStock(content) {
    const s = LJ.Sys.state;
    const open = marketOpen();
    content.appendChild(el(`<div class="panel-note">📈 A股模拟交易。交易时间：周一至周五 9:30-11:30 / 13:00-15:00。T+1，单日涨跌幅±10%。${open ? '' : '（当前休市）'}</div>`));
    const box = el(`<div class="fx-box">
      <div class="row">股票账户资金：<b>${LJ.Engine.fmtMoney(s.stock.cash)}</b>
        <input type="number" id="stockAmt" min="0" value="100"><button id="stockIn">转入</button><button id="stockOut">转出</button></div>
    </div>`);
    ...
    content.appendChild(box);
    const grid = el(`<div class="buy-table">${LJ.Stocks.map((st) => {
      ...
      return `<div class="buy-row">
        <div class="buy-name">${st.name}<small>${st.sector} · 持仓${pos}股</small></div>
        <div class="buy-price"><b>${price}元</b></div>
        <div class="buy-num"><input type="number" min="1" value="100" data-stock-num="${st.id}"></div>
        <div class="buy-op"><button data-stock-buy="${st.id}" ${open ? '' : 'disabled'}>买入</button><button data-stock-sell="${st.id}" ${open && t1 && pos > 0 ? '' : 'disabled'}>卖出</button></div>
      </div>`;
    }).join('')}</div>`);
    $all('button', grid).forEach((b) => {
      ...
      b.addEventListener('click', () => LJ.Finance.stockTrade(id, qty(), b.dataset.stockBuy ? 'buy' : 'sell'));
    });
  ...
  LJ.Finance = { tickInterest, renderBank, renderStock, stockTrade, marketOpen };
})();
